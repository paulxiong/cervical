...
"""
#==========================
# Add by wzy for some test
#==========================
def median_norm(image, iters=1):
    eps = 1e-6
    median = np.median(image)
    
    def norm(array, threshold):
        min_a = np.min(array)
        max_a = np.max(array)
        new_array = np.round((array - min_a)/(max_a - min_a + eps) * threshold)
        return new_array.astype(np.uint8)
    
    for i in range(iters):
        image[image <= median] = norm(image[image <= median], 127)
        image[image > median] = norm(image[image > median], 127) + 128
...
def rgb_clahe_justl(in_rgb_img):
    ...
    # add by wzy, median_norm can be iterated for times
    #ch0 = median_norm(lab[:,:,0], 2)
    #ch0 = lab[:,:,0]
    #++++++++++++++++++++
...
def rgb_he_justl(in_rgb_img):
    bgr = in_rgb_img[..., ::-1]
    lab = cv2.cvtColor(bgr, cv2.COLOR_BGR2LAB)
    img = cv2.equalizeHist(lab[:,:,0])
    return img
    
def get_l_channel(in_rgb_img):
    bgr = in_rgb_img[..., ::-1]
    lab = cv2.cvtColor(bgr, cv2.COLOR_BGR2LAB)
    return lab[..., 0]
...
def process_images1(img_df):
    """It adds a column to the given data frame for storing processed images
    It amplifies regional attributes"""
    img_df['image_process'] = img_df['image'].map(lambda x: cv2.bitwise_not(rgb_clahe_justl(x))[:,:, np.newaxis])
    #img_df['image_process'] = img_df['image'].map(lambda x: cv2.bitwise_not(get_l_channel(x))[:,:, np.newaxis])
    return img_df
...
def preprocess_image(image):
    ...
    #invert image of private cervical cells
...
def post_process_image(image, mask, contour):
    ...
    try:
        ...
        #print('Use random_walker algorithm to generate labels')
        labels = random_walker(mask_plus_contour, labels, multichannel=True)   

    except:
        #print('Use watershed algorithm to generate labels')
        labels = cv2.watershed(mask_plus_contour, labels)
    '''
    #random_walker is in sklearn, maybe slow ,try cv2 methods
    labels = cv2.watershed(mask_plus_contour, labels)
