...
def GetBlurryScore(img):
    img = img.squeeze()
    if np.shape(img)[-1] == 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    return cv2.Laplacian(img, cv2.CV_64F).var()
    
def split_image_to_windows1(image, window_height, window_width, overlap=True, overlap_corners=True):
    """Split an input image into multiple equal-sized windows.
    ...
    if not overlap_corners:
        ...
        for h in range(num_height_windows):
            for w in range(num_width_windows):
                ...
                img = image[h * src_window_height:image_edge_height,
                            w * src_window_width:image_edge_width,:]
                
                #filtered the blur windows, and only segment clear windows
                score = GetBlurryScore(img)
                #print('Score no overlap corner: {}'.format(score))
                if score > 190:
                    window_images[cur_window] = np.pad(img,((0,window_pad_height),(0,window_pad_width),(0,0)),'symmetric')
                cur_window += 1
    else:
        cur_window = 0
        for h in range(num_height_windows):
            ...
            for w in range(num_width_windows):
                ...
                if w == 0:
                    pad_left = src_window_width
                pad_right = window_width - pad_left - (src_right - src_left)
                img = image[src_top:src_bottom,
                            src_left:src_right,:]
                score = GetBlurryScore(img)
                #print('Score overlap corner: {}'.format(score))
                #filtered the blur windows, and only segment clear windows
                if score > 190:
                    window_images[cur_window] = np.pad(img,((pad_top,pad_bottom),(pad_left,pad_right),(0,0)),'symmetric')
