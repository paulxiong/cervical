...
"""import modules for running time analysis"""
import time
import numpy as np
import re
...
def time_it(fn):
    def new_fn(*args,**kwargs):
        ...
        name = re.search(r'[a-zA-Z_]*\.[a-zA-Z_]*',str(fn)).group()
        #print(name)
        #cp_list.append([name,duration])
        return result
    return new_fn

def apply_mp_wrapper(df,image_column='image_process'):
    res = df.apply(lambda x: convert_prediction_to_labels(x['prediction'], x[image_column], x['size']),axis=1)
    #print(type(res),res)
    if not res.empty:
        res = res.map(lambda x: x[0])
        return res
    else:
        return None
...
class NucleiUtility(object):
    def __init__(self):
        self.OUTPUT_PATH = '../../../../datasets/segment/output/'
        ...
        self.TRAIN_PATH = '../../../../datasets/segment/stage1_train/'
        self.TEST_PATH = '../../../../datasets/segment/test/'
        self.RETRAIN = False
        self.GPU = 1
        self.EPOCH = 1
        self.KFOLD = -1
    ...
    def parse_argument(self, arguments=None):
        ...
        parser.add_argument('action', type=str,
                            choices=['train', 'predict', 'loadpredict','predict_test'],
        ...
        parser.add_argument('--gpu',type=int, nargs=1, metavar='<number of GPUs>',
        ...
        parser.add_argument('--outputpath', nargs=1, metavar='<output data set path>',
                            help='Specify the file path of the output')
        ...
        try:
            ...
            if args.outputpath is not None:
                if not os.path.isdir(args.outputpath[0]):
                    ...
                    self.logger.error('The output path \'' + args.outputpath[0] + '\' you specified does not exist\n')
                else:
                    self.OUTPUT_PATH = args.outputpath[0]
    ...
    def preprocess_images(self, train_img_df, test_img_df, num_clusters = 2):
        process_images1(train_img_df)
        process_images1(test_img_df)

        cluster_train_df_list = split_cluster_to_group(train_img_df, num_clusters)
        #cluster_train_df_list = [train_img_df, []]
        if len(test_img_df) == num_clusters:
            cluster_test_df_list = split_cluster_to_group(test_img_df, num_clusters)
        else:
            cluster_test_df_list = [test_img_df, []]

        self.set_cluster_index(cluster_test_df_list)
    ...
    def predict(self, cluster_train_df_list, cluster_val_df_list, cluster_test_df_list, grey_models=None, colour_models=None):
        ...
        '''only colour_model[0] used to predict result'''
        self.predict_model(None, [colour_models[0]], cluster_test_df_list, 'test')
    ...
    def create_submission(self, cluster_test_df_list):
        ...
        if 'label' in cluster_test_df_list[self.COLOUR_IX]:
    ...
    '''------------------------------'''
    """add by Zhuoyao for test prediction"""
    '''------------------------------'''
    @time_it
    def add_labels(self, cluster_df, image_column='image_process'):
        ...
        splits_df = np.array_split(cluster_df,cpus)

        print('Start multiprocessing for pandas apply')
        result = p.map(apply_mp_wrapper,splits_df)
        p.close()
        p.join()

        labels = pd.concat(result, axis=0)'''

        splits_df = np.array_split(cluster_df, cpus)
        res_q = queue.Queue()

        def run(df, t_id):
            res_q.put(apply_mp_wrapper(df))
            print("Thread %s finished!" % t_id)

        threads = []
        for t_id in range(cpus):
            thread_alive = thr.Thread(target=run, args=(splits_df[t_id], t_id))
            thread_alive.start()
            threads.append(thread_alive)

        for thread_alive in threads:
            thread_alive.join()
        ...
        labels = pd.concat(results, axis=0)
        labels = labels.sort_index()

        #import pdb
        #pdb.set_trace()

        assert list(labels.index) == list(cluster_df.index), 'Label index is not equal to orginal index,take care of sequences'
    ...
    @time_it
    def preprocess_test_images(self, test_img_df, num_clusters = 2):
        process_images(test_img_df)

        cluster_test_df_list = split_cluster_to_group(test_img_df, num_clusters)

        self.logger.debug('Grey train images size: %d', len(cluster_test_df_list[self.GREY_IX]))
        self.logger.debug('Colour train images size: %d', len(cluster_test_df_list[self.COLOUR_IX]))

        self.logger.debug(cluster_test_df_list[0].sample())
        return cluster_test_df_list

    @time_it
    def test_batched_predictions(self, img_df, models):
        # Iterator to chunk images/sizes into batches.
        def image_batches(images, sizes, batch_size):
            for i in range(0, len(images), batch_size):
                yield (images[i:i + batch_size], sizes[i:i + batch_size])
        ...
        # windowing and rotation), trying to do all images at once encountered a Python MemoryError.
        batch_size = 100
        self.logger.info('Predict on test images in batches of up to %d original images...', batch_size)
        for (batch, sizes) in image_batches(all_images, all_sizes, batch_size):
            #only split images into windows, no rotation or flip
            predict_inputs = window_images1(batch,self.IMG_HEIGHT, self.IMG_WIDTH)
            predictions = average_model_predictions(predict_inputs, models)
            #because no rotation and flip, only need to stitch the windows
            predictions_full_size.extend(stitch_all_images(predictions, sizes))
            del predictions

        return predictions_full_size

    @time_it
    def predict_model_for_test(self, img_df, models, predict_path):
        self.logger.info('Performing batched test predictions for color images...')
        predictions_full_size = self.test_batched_predictions(img_df, models)

        img_df['prediction'] = pd.Series(predictions_full_size).values
        predictions_full_size = None

        self.logger.info('Save predictions of test color images under %s', os.path.join(self.OUTPUT_PATH, predict_path))
        save_prediction_images(img_df, os.path.join(self.OUTPUT_PATH, predict_path))

        #self.add_labels(img_df)

        #print(img_df.head(1))

    @time_it
    def predict_test(self, cluster_df_list, colour_models=None, grey_models=None):
        if self.COLOUR_ONLY:
            print('dataframe shape:',cluster_df_list[self.COLOUR_IX].shape)
            print(cluster_df_list[self.COLOUR_IX].columns)

            if colour_models is None and self.LOAD_MODELS_COLOUR_PATH:
                colour_models = load_models(self.LOAD_MODELS_COLOUR_PATH, self.GPU)
                print(get_flops(colour_models))

            predict_colour_path = self.PREDICT_TEST_COLOUR_PATH

            self.predict_model_for_test(cluster_df_list[self.COLOUR_IX],
                                                colour_models, predict_colour_path)

            self.logger.debug('Sample the final test data frame after prediction to make sure everything is good')
            #self.logger.debug(cluster_df_list[self.GREY_IX].sample())
            self.logger.debug(cluster_df_list[self.COLOUR_IX].sample())

        if self.GREY_ONLY:
            print('dataframe shape:',cluster_df_list[self.GREY_IX].shape)
            print(cluster_df_list[self.GREY_IX].columns)

            if grey_models is None and self.LOAD_MODELS_GREY_PATH:
                grey_models = load_models(self.LOAD_MODELS_GREY_PATH, self.GPU)
                print(get_flops(grey_models))

            predict_grey_path = self.PREDICT_TEST_GREY_PATH

            self.predict_model_for_test(cluster_df_list[self.GREY_IX],
                                                grey_models, predict_grey_path)
...
def step2v2(action, modpath, cuda_device, datasets_train_path, datasets_test_path, output_path):
    ...
    util.LOAD_MODELS_GREY_PATH = os.path.join(modpath,  util.MODELS_GREY_PATH)
    util.ROTATE_IMAGES = False
    util.INVERT_IMAGES = False
    util.VISUALIZE = False
    util.MOSAIC = True
    util.PREDICT_TEST_IMAGES_ONLY = True
    util.GREY_ONLY = False
    util.COLOUR_ONLY = True
    ...
    if action == 'train':
        #util.TEST_PATH = '../../../../datasets/segment/stage1_test'
    ...
    #    print('Function {} time cost: {} ms'.format(name,duration))
    #fd = open('./timer.txt','w')
