...
def validate1(args, fixed_z, fid_stat, gen_net: nn.Module, writer_dict):
    writer = writer_dict['writer']
    global_steps = writer_dict['valid_global_steps']
   ...
   # print("sample_imgs:",sample_imgs)
    ...
    for iter_idx in tqdm(range(eval_iter), desc='sample images'):
        ...
        gen_imgs = gen_net(z).mul_(127.5).add_(127.5).clamp_(0.0, 255.0).permute(0, 2, 3, 1).to('cpu', torch.uint8).numpy()
        for img_idx, img in enumerate(gen_imgs):
            file_name = os.path.join(fid_buffer_dir1, f'iter{iter_idx}_b{img_idx}.png')
