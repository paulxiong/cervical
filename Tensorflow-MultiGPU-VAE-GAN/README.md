...
```
### product more cell !!!(重要)
```
目前vae-gan的输入为真实细胞数据，且输入的真实数据与输出的假数据比例为1:1，当输入为其他类型细胞或者随机噪声，输出的假细胞数据质量很差。为了能够产生更多的数据，可以将输入数据等分成N份，用第一份进行训练得出模型，用2-N份真实数据作为vae-gan输入，生成N-1份假细胞数据，再用第二份进行训练得出模型，用1和3-N份作为vae-gan输入，生成N-1份数据，以此类推生成更多假细胞数据。但生成的假细胞数据仍需要经过人工筛选。
```
