<template>
  <div class="photo-zoom-pro">
    <div
      ...
      @mouseleave="!disabled && !leaveEvent && mouseMove($event)"
      @mouseup="!disabled && !upEvent && mouseMove($event)"
    >
      <img ref="img" class="origin-img" @load="imgLoaded($event)">
      <div
        v-if="selector"
    ...
    mouseMove(e) {
      ...
      if (this.imgLoadedFlag && e) {
        ...
        if (e.type === 'mouseup' || e.type === 'mouseleave') {
          const imgheight = (zoomerRect.top + this.height / 2) / this.imgInfo.height
          const imgwidth = (zoomerRect.left + this.width / 2) / this.imgInfo.width
          this.$emit('updatexy', { 'xpercent': imgwidth, 'ypercent': imgheight })
        }
      }
