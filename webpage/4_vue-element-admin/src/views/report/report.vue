<template>
  <div class="reportData">
    ...
    <el-dialog title="新增报告" :visible.sync="dialogFormVisible">
      <newReport ref="newReport" style="margin-top:-40px;" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      this.getListreport(this.currentPageSize, (this.currentPage - 1) * val, 1)
    },
    handleSizeChange(val) {
      this.currentPageSize = val
      this.getListreport(val, (this.currentPage - 1) * val, 1)
    },
    goDetail(val) {
      this.$router.push({
        path: '/'
