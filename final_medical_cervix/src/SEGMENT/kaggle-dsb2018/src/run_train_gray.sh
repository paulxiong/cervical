...
function train()
{
    ...
    if [ -n "$2" ]; then
        case "$2" in
            public) TRAIN_PATH_FLAG=;TRAIN_PATH=; EPOCH=20;\
                    OUTPUT=./${1}_output;MODEL_FLAG=; MODEL=;;
            private) TRAIN_PATH_FLAG=--trainpath; \
                     TRAIN_PATH=../../../../datasets/segment/private_train; \
                     EPOCH=10; OUTPUT=./${1}_output; \
                     MODEL_FLAG=--loadmodel; MODEL=./${1}_output ;;
        esac
    fi
    echo $1
    echo $2
    ...
    python3 schwaebische_nuclei.py train \
	    ...
	    --output $OUTPUT \
       	--colouronly \
	    --cuda_device $CUDA \
	    $TRAIN_PATH_FLAG $TRAIN_PATH $MODEL_FLAG $MODEL $C_FLAG $C_VALUE
}
