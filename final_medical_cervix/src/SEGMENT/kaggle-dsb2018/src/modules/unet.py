...
def train_model(X_train, Y_train, X_val, Y_val, epochs=20, gpu=1, earlystopping=5, load_model_path=None, save_model_path=None, min_dropout=0.1):
    ...
    if (len(X_val) > 0) and (len(Y_val) > 0):
        ...
        typed_model.fit(X_train, Y_train, validation_data = (X_val, Y_val), epochs=epochs, batch_size=int(gpu*32), callbacks=[earlystopper], verbose=1)
        ...
        typed_model.fit(X_train, Y_train, epochs=epochs, batch_size=int(gpu*32), verbose=1)
...
def train_model_kfold(X_train, Y_train, n_splits, epochs=20, gpu=1, earlystopping=5, load_models_path=None, save_model_path=None, min_dropout=0.1):
    ...
    for index, (train, val) in enumerate(kfold.split(X_train)):
        ...
        typed_model.fit(X_train[train], Y_train[train], validation_data=[X_train[val], Y_train[val]], epochs=epochs, batch_size=int(gpu*32), callbacks=[earlystopper], verbose=1)
