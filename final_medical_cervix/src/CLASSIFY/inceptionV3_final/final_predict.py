...
def load_train(path):
    ...
    train_label = np.array(train_set['positive'].iloc[: ])
    train_files = []
...
PREVIEW_FOLDER = '../../../datasets/classify/{}.png_output/preview/'.format(sys.argv[1])
final_result_text = '{}/final_result.csv'.format(PREVIEW_FOLDER)

# Png is better
INPUT_FILE_PATTERN = '*.png'
classify_text = "input/sample_submission.csv"
...
test_set.to_csv(final_result_text, index = None)
