...
def up_sample(df, choices, tmp_dir, with_origin=False):
    ...
    for file, gt in df[['name', 'gt']].values:
        ...
        for choice in choices:
            img_aug = augment(img, choice)
            save_path = os.path.join(aug_path, str(choice)+'_'+base_name)
...
def dataset_augment(df, tmp_dir='./tmp'):
    ...
    if AUGMENTATION == 'Minority':
        up_sample_df = up_sample(pos_samples, range(6), tmp_dir)
        df = df.append(up_sample_df, ignore_index=True)
    elif AUGMENTATION == 'Majority_Down_Minority_Up':
        df = down_sample(neg_samples, RATIO)
        choices = np.random.choice(6, 3, replace=False)
        up_sample_df = up_sample(pos_samples, choices, tmp_dir, with_origin=True)
        df = df.append(up_sample_df, ignore_index=True)
        ...
        up_sample_df = up_sample(df, range(6), tmp_dir)
        df = df.append(up_sample_df, ignore_index=True)
    else:
...
def load_train_from_folder(path):
    ...
    for c, gt in zip(CLASS, GTS):
        files = glob.glob(os.path.join(path, c, '*.png'))
        #print(files)
        if len(files) != 0:
...
def dataset_div_by_crop_aspect(df, threshold=150):
    df_area = pd.DataFrame(df.apply(lambda x: cv2.imread(x['name']).shape[:-1], axis=1).tolist(), columns=['h','w'])
    df = pd.concat([df, df_area], axis=1)
    df_bigger = df.loc[(df['w'] >= threshold) | (df['h'] >= threshold)]
    df_smaller = df.loc[(df['w'] < threshold) & (df['h'] < threshold) ]
    return df_bigger, df_smaller
#====================
# main
#====================
if __name__ == '__main__':
    ...
    train_set, _ = dataset_div_by_crop_aspect(train_set,threshold=40)
    test_set, _ = dataset_div_by_crop_aspect(test_set,threshold=40)
